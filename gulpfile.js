...
const bucket = 'crypto-dashboard'
const distrId = 'E2Y30ZPUZQEKYN'
...
gulp.task('s3versionCheck', (cb) => {
  s3.listObjects({ Bucket: bucket, Prefix: `v${version}` }, (err, data) => {
    if (err) throw new Error(`[AWS-ERROR]: could not list ths bucket's (${bucket}) contents. \n${err}`)
...
gulp.task('deploy', (cb) => runSequence(
  's3versionCheck',
  's3deploy',
  'updateDistr',
